...
function GeneralDataSection({ city }: Props) {
  if (!city) return null;
  const {
    data: weeklyData,
  ...
  if (loadingWeekly || loadingHourly) {
    ...
    return (
      <Card className="w-90 h-20 md:absolute top-[20%] left-[35%] flex items-center justify-center text-destructive font-bold">
    ...
    <>
      <section className="flex items-center justify-around gap-4 mb-10">
        <div className="flex flex-col items-center">
          <span className="text-secondary-foreground text-sm">{today?.datetime}</span>
          ...
          <WeatherIcon icon={nowHour.icon} size={180} />
      </section>

      <section className="grid grid-cols-2 gap-2">
        {nowHour?.precipprob > 0 && (
          <Card className="flex items-center justify-center glass-effect text-sm md:text-xl">
